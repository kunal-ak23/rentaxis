    ...
    return (
        <div className="p-8 max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
                <div>
                    <Link href="/dashboard/properties" className="flex items-center gap-1.5 text-[10px] font-black text-primary uppercase tracking-widest mb-4 hover:-translate-x-1 transition-transform">
                        <ArrowLeft size={12} />
                        Back to Projects
                    </Link>
            ...
            {showForm && (
                <div className="fixed inset-0 bg-gray-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
                    <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl border border-gray-100 relative">
                        <button onClick={() => setShowForm(false)} className="absolute right-6 top-6 p-2 text-gray-400 hover:text-gray-600 transition-colors"><X size={18} /></button>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input required placeholder="e.g. 101" className="w-full bg-input border border-border p-3 rounded-xl text-xs" value={formData.unitNumber} onChange={ev => setFormData({ ...formData, unitNumber: ev.target.value })} />
                                </div>
                                <div>
                                    <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5 ml-1">{t("status")}</label>
                                    <select className="w-full bg-input border border-border p-3 rounded-xl text-xs" value={formData.status} onChange={ev => setFormData({ ...formData, status: ev.target.value })}>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" placeholder="AED" className="w-full bg-input border border-border p-3 rounded-xl text-xs" value={formData.expectedRent || ""} onChange={ev => setFormData({ ...formData, expectedRent: Number(ev.target.value) })} />
                                </div>
                                <div>
                                    <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5 ml-1">{t("sizeSqft")}</label>
                                    <input type="number" placeholder="Sq. Ft." className="w-full bg-input border border-border p-3 rounded-xl text-xs" value={formData.sizeSqft || ""} onChange={ev => setFormData({ ...formData, sizeSqft: Number(ev.target.value) })} />
                                </div>
                            </div>
                            <div className="flex justify-end gap-3 mt-4">
                                <button type="button" onClick={() => setShowForm(false)} className="px-6 py-3 rounded-xl text-xs font-bold text-gray-500">{t("cancel")}</button>
                    ...
                    <CardFlip
                        ...
                        back={
                            <div className="h-full flex flex-col">
                                ...
                                <button className="mt-4 w-full py-2.5 rounded-xl bg-primary text-white text-[10px] font-black uppercase tracking-widest hover:opacity-90 transition-all">
                                    Edit Details
                                </button>
                            </div>
                        }
            ...
            {units.length === 0 && (
                <div className="p-20 text-center flex flex-col items-center bg-gray-50/50 border border-dashed border-gray-200 rounded-[3rem]">
                    ...
                    <button onClick={() => setShowForm(true)} className="text-xs font-black text-primary border-b-2 border-primary pb-0.5">
                        {t("addProperty")}
                    </button>
                </div>
            )}
