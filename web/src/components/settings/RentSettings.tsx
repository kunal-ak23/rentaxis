...
export default function RentSettings({ embedded = false, hideOnlinePaymentToggle = false }: { embedded?: boolean; hideOnlinePaymentToggle?: boolean } = {}) {
    ...
    const handleSave = async () => {
        ...
        try {
            const res = await fetch(`/api/proxy/v1/rent-settings/${selectedPropertyId}`, {
                ...
                body: JSON.stringify(toRentSettingsBody(settings)),
            });
            if (res.ok) {
                setSaveSuccess(true);
                const saved = await res.json();
                setSettings({ ...DEFAULT_RENT_SETTINGS, ...saved });
    ...
    const updateField = <K extends keyof RentSettingsData>(field: K, value: RentSettingsData[K]) => {
        if (!settings) return;
        setSettings({ ...settings, [field]: value });
    };

    // Fine override helpers
    const setFineOverride = (field: keyof RentSettingsData, value: string) => {
        if (!settings) return;
        const parsed = value === "" ? null : Number(value);
        setSettings({ ...settings, [field]: parsed });
    };

    const resetFineOverride = (field: keyof RentSettingsData) => {
        if (!settings) return;
        setSettings({ ...settings, [field]: null });
    };

    const fineOverrideValue = (field: keyof RentSettingsData): string => {
        if (!settings) return "";
        const val = settings[field];
        return val === null || val === undefined ? "" : String(val);
    };

    const isOverridden = (field: keyof RentSettingsData): boolean => {
    ...
    return (
        <div className="max-w-4xl">
            {/* Page Header */}
            <div className="mb-8">
                <Heading className="text-2xl font-bold text-foreground tracking-tight flex items-center gap-3">
            ...
            {settings && !loading && (
                <div className="bg-surface rounded-xl p-5 border border-border hover:shadow-md transition-all duration-200 mb-6">
                    ...
                    <div className="space-y-6">
                                ...
                                {[
                                    ...
                                    >
                                        <input
                                            ...
                                            onChange={e => updateField("penaltyType", e.target.value as RentSettingsData["penaltyType"])}
                        ...
                        {/* Online Payment Toggle — in Settings › Payments it has its own switch (OnlinePaymentSwitch) */}
                        {!hideOnlinePaymentToggle && (
