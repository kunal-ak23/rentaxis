...
export default function OnlinePaymentSwitch() {
    ...
    // Only the latest selection's response may land: picking A then B quickly
    // must never leave A's row in state, because the save sends the whole row
    // back to whichever property is selected (PR #363 R1).
    ...
    const selectProperty = (id: string) => {
        ...
        if (!id) return;
        fetch(`/api/proxy/v1/rent-settings/${id}`)
            .then(res => readRentSettings(res, id))
            .then(loaded => { if (seq === requestSeq.current) setSettings(loaded ? { ...loaded, propertyId: id } : null); })
            .catch(() => { if (seq === requestSeq.current) setSettings(null); });
    };
    /** The loaded row belongs to the selected property — the only state a save is allowed from. */
    const loadedForSelected = settings !== null && settings.propertyId === propertyId;
    ...
    return (
        <section className="bg-surface rounded-xl border border-border p-5 space-y-4" data-testid="online-payment-switch">
            ...
            <select value={propertyId} onChange={e => selectProperty(e.target.value)} aria-label={t("selectProperty")}
            ...
            {settings && loadedForSelected && (
                <div className="flex items-center justify-between bg-input rounded-xl p-4 border border-border">
                    ...
                    <button type="button" role="switch" aria-checked={settings.onlinePaymentEnabled} aria-label={tOp("toggleOnlinePayment")}
                        disabled={status === "saving" || !loadedForSelected} onClick={toggle}
