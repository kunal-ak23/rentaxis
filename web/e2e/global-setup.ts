...
setup('seed test data', async () => {
  ...
  const usersToCreate = [
    { name: 'E2E Tenant Admin', email: `e2e-ta-${suffix}@test.com`, password: 'test123', role: 'TENANT_ADMIN' },
    { name: 'E2E Property Manager', email: `e2e-pm-${suffix}@test.com`, password: 'test123', role: 'PROPERTY_MANAGER' },
    { name: 'E2E Tenant User', email: `e2e-tu-${suffix}@test.com`, password: 'test123', role: 'TENANT_USER' },
  ];
  ...
  const renterPassword = renterId ? `Renter@${renterId.substring(0, 6)}` : 'test123';
  ...
  const testContext = {
    ...
    users: {
      ...
      tenantAdmin: { email: usersToCreate[0].email, password: 'test123' },
      propertyManager: { email: usersToCreate[1].email, password: 'test123' },
      tenantUser: { email: usersToCreate[2].email, password: 'test123' },
