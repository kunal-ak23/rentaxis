...
| 02. Dashboard, search, notifications, and help | Web, Manager, Renter | All | KPI dashboard, global search, alerts, notification center, help articles, guided tours | Prod E2E (notification delivery); production browser journeys prepared for exact notification inbox/unread/read state, KPI dashboard, command-palette lease navigation, Security Guard help detail, and Super Admin follow-ups; #100/#101 are merged, while #102/#108 still need merge and deployment before the run; Local E2E |
...
- Audited mobile baseline: 512/512 Flutter unit/widget tests passed before the app-version gate landed (core 94, manager 100, renter 224, security 94); Manager, Renter, and Security analyzers reported no issues. #110 independently verifies the current rebased Manager source with five additional route tests and a clean analyzer.
...
- Notification inbox PR: [#108](https://github.com/kunal-ak23/rentaxis/pull/108) immediately removes read/read-all items from the Unread tab and keeps its paging state consistent; 6 focused component tests, TypeScript, and the webpack production build pass.
- Current `origin/main` baseline `c8ac199` includes #95, #98–#101, #104–#107, and the tutorial assets from #106. The remaining open work is production E2E coverage #96, search #102, notification inbox #108, Manager route tests #110, and public registration #111, plus this route-map verifier.
- Safe full production rerun order: merge and deploy the remaining product fixes #102, #108, and #111; confirm all already-merged safety fixes are in production; then explicitly approve, implement, test, and deploy exact cleanup for non-contract uploaded artifacts before executing #96 against a disposable TEST-E2E tenant. #110 and this route-map change are test/documentation-only. The real cheque-upload scenario creates an Azure blob that tenant deletion does not currently remove. No gated feature was changed on the persistent tutorial tenant.
