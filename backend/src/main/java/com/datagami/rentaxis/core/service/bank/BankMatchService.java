...
public class BankMatchService {
    ...
    public BankRecDTOs.Match manual(BankRecDTOs.ManualMatchInput in) {
        ...
        if (in != null && in.openingItemIds() != null && !in.openingItemIds().isEmpty()) {
            throw BankRecRefusal.refuse("openingItemsNotHere", "Opening items arrive with reconciliations; match journal lines here");
        }
        if (ls.isEmpty() && js.isEmpty()) throw BankRecRefusal.refuse("selectToMatch", "Select statement lines and book items to match");
