...
| 2.4 | L123 "contractNumber"; used at plan L5891 `lease.setContractNumber("TLP7/681")` and L6458 `lease.getContractNumber()` in a string concat | `Lease.java:90` — **`private Long contractNumber`**. The v2 sheet's `ContractNumber` is alphanumeric (`TLP7/681`). | **differs — compile error at L5891**, and a genuine data-model gap: there is nowhere on `Lease` to store PACT's contract string. Needs a new column or a decision to drop it. |
