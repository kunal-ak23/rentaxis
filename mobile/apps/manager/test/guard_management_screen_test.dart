...
void main() {
  ...
  group('Guard list', () {
    testWidgets(
      'lists only SECURITY_GUARDs, with the phone they sign in with',
      (tester) async {
  ...
  group('Create guard', () {
    testWidgets('a malformed phone is rejected without a network call', (
      ...
      await tester.enterText(
        find.byKey(const Key('guard-phone')),
        '0501234567',
      );
      await tester.enterText(
        find.byKey(const Key('guard-email')),
        'rakesh@example.com',
      );
    ...
    testWidgets('a valid guard is posted and the roster re-read', (
      ...
      expect(
        find.textContaining('sign in with their phone number'),
        findsOneWidget,
      );
    });
    ...
    testWidgets('a duplicate-phone rejection reads as a human message', (
      ...
      expect(
        find.textContaining('already exists in this tenant'),
        findsNothing,
      );
    });
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(
          find.textContaining('already registered to Rakesh Kumar'),
          findsOneWidget,
        );
        expect(admin.created, isEmpty);
      },
  ...
  group('Property assignment', () {
    testWidgets(
      ...
      (tester) async {
        ...
        final admin = FakeGuardAdminService(
          userRows: [userFixture(id: 'g-1', name: 'Rakesh Kumar')],
        );
    ...
    testWidgets('a failed save says the posting is unchanged', (tester) async {
      final gatePass = FakeGatePassService(
        ...
        setGuardPropertiesError: httpError(
          400,
